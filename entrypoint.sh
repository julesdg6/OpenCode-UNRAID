...
OPENCODE_READINESS_TIMEOUT="${OPENCODE_READINESS_TIMEOUT:-60}"
...
wait_for_opencode() {
  local retries="$OPENCODE_READINESS_TIMEOUT"
  while [ "$retries" -gt 0 ]; do
    if ! kill -0 "$opencode_pid" 2>/dev/null; then
      echo "opencode serve exited before becoming ready" >&2
      return 1
    fi
    if curl -fsS --max-time 1 -o /dev/null "http://127.0.0.1:${OPENCODE_PORT}" 2>/dev/null; then
      return 0
    fi
    retries=$((retries - 1))
    sleep 1
  ...
  echo "Timed out waiting ${OPENCODE_READINESS_TIMEOUT}s for opencode serve on 127.0.0.1:${OPENCODE_PORT}" >&2
  return 1
}
